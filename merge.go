...
func mergeSequence(node *yaml.Node, depth uint8) error {
	...
	case yaml.DocumentNode:
		return mergeSequence(node.Content[0], depth+1)
	...
	case yaml.MappingNode:
		...
		for i := len(node.Content); i > 1; i = i - 2 {
			if err := mergeSequence(node.Content[i-1], depth+1); err != nil {
				return err
			}
		}
	...
	case yaml.SequenceNode:
		...
		for i := range node.Content {
			...
			if node.Content[i].Kind == yaml.MappingNode &&
				...
				if err := mergeSequence(node.Content[i], depth+1); err != nil {
					return err
				}
